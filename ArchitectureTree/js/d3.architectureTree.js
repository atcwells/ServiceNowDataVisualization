...
d3.chart.architectureTree = function () {
    ...
    var updateData = function (container, nodes, links) {
        ...
        node.append("circle")
            ...
            .style('fill', function (d) {
                if (typeof d.satisfaction === "undefined")
        ...
        return function (node) {
            ...
            svg.selectAll(".node")
                .filter(function (d) {
                    ...
                    if ((d.group === node.group
                        && d.level > node.level
                        && d.ancestorIdList
                        && d.ancestorIdList.indexOf(node.id) > -1)
                        || d.name == node.name)
                        return false;
    ...
    var isFoundByFilter = function (d) {
        ...
        if (filters.name) {
            if (d.name.toLowerCase().indexOf(filters.name) === -1)
                return false;
        }
        var technosCount = filters.technos.length;
        if (technosCount) {
            if (d.index.technos.length === 0)
                return false;
            for (i = 0; i < technosCount; i++) {
                if (d.index.technos.indexOf(filters.technos[i]) === -1)
                    return false;
            }
        }
        ...
        if (hostCount) {
            ...
            for (i = 0; i < hostCount; i++) {
                if (d.index.hosts.indexOf(filters.hosts[i]) === -1)
                    return false;
            }
        }
        return true;
